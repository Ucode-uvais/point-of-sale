...
describe("offline checkout helpers", () => {
  ...
  it("creates client request ids with sanitized shop and user parts", () => {
    const value = new Date("2026-04-06T13:45:20+08:00");
    const id = createOfflineClientRequestId(
      "shop-001/main",
      "cashier#1",
      value,
    );
    ...
    expect(id).toMatch(
      /^OFFSYNC-SHOP001MAI-CASHIER1-20260406134520-[A-Z0-9]{8}$/,
    );
  });
  ...
  it("computes stock snapshot age in minutes and handles invalid values", () => {
    ...
    expect(getStockSnapshotAgeMinutes("2026-04-06T13:45:00+08:00", now)).toBe(
      15,
    );
    expect(getStockSnapshotAgeMinutes("not-a-date", now)).toBe(
  ...
  it("reads, writes, and removes localStorage values in browser-like environments", () => {
    ...
    expect(readLocalStorageValue(key, { ok: false })).toEqual({
      ok: true,
      count: 2,
    });
