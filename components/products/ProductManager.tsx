  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    ...
    for (const variant of form.variants) {
      if (
        ...
        setError(
          "Each variant needs at least one descriptor, SKU, or barcode.",
        );
        return;
      }
    ...
    <div className="space-y-6">
      {canEditProducts ? (
        <Card>
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-500">
                Set product pricing, structured variants, media, and
                barcode-ready merchandising details while keeping stock changes
                on their audited flows.
              </p>
            </div>
          ...
          <form onSubmit={saveProduct} className="space-y-6">
            <div className="rounded-[26px] border border-stone-200 bg-stone-50/80 p-4 sm:p-5">
              <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-400">
                Core details
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...
                        uomConversions: current.uomConversions.filter(
                          (conversion) =>
                            conversion.unitOfMeasureId !== event.target.value,
                        ),
                      }))
                    }
                  >
                ...
                <div>
                  <FieldLabel>
                    {editingId ? "Current stock" : "Opening stock quantity"}
                  </FieldLabel>
                  <Input
                    type="number"
                    value={form.stockQty}
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Optional reason for pricing or cost updates"
                  />
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_0.9fr]">
                <div
                  className={`rounded-[20px] border px-4 py-3 text-sm ${currentMargin.tone === "red" ? "border-red-200 bg-red-50 text-red-700" : currentMargin.tone === "amber" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}
                >
                  Margin check: {currentMargin.message} Current margin{" "}
                ...
                <div className="rounded-[20px] border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600">
                  ...
                  <span className="font-semibold text-stone-900">
                    {selectedBaseUnit?.name ?? "Not selected"}
                  </span>
                  {" / "}
            ...
            <div className="rounded-[26px] border border-stone-200 bg-white/70 p-4 sm:p-5">
              ...
              <div className="mt-4 rounded-[20px] border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-600">
                {inventoryDefaults.batchTrackingEnabled
                  ? "Batch tracking is enabled by default for this shop."
                  : "Batch tracking is optional in this shop."}{" "}
                {inventoryDefaults.expiryTrackingEnabled
                  ? "Expiry tracking is enabled by default."
                  : "Expiry tracking is optional."}
              </div>
              ...
              <div className="mt-4 rounded-[20px] border border-stone-200 bg-white p-4">
                ...
                <div className="space-y-3">
                  {units
                    ...
                    .map((unit) => {
                      const conversion = form.uomConversions.find(
                        (entry) => entry.unitOfMeasureId === unit.id,
                      );

                      return (
                        <div
                          key={unit.id}
                          className="grid gap-3 sm:grid-cols-[1fr_180px] sm:items-center"
                        >
                          <div className="text-sm text-stone-600">
                            ...
                            {selectedBaseUnit?.name.toLowerCase() ??
                              "base unit"}
                            {conversion?.ratioToBase === "1" ? "" : "s"}
                          </div>
                          <Input
                            type="number"
                            min="1"
                            ...
                            onChange={(event) =>
                              setForm((current) => {
                                ...
                                const remaining = current.uomConversions.filter(
                                  (entry) => entry.unitOfMeasureId !== unit.id,
                                );
              ...
              <div className="space-y-4">
                {form.variants.length ? (
                          ...
                          onChange={(event) =>
                            setForm((current) => ({
                              ...
                              variants: current.variants.map(
                                (entry, entryIndex) =>
                                  entryIndex === index
                                    ? { ...entry, flavor: event.target.value }
                                    : entry,
                              ),
                            }))
                          }
                      ...
                      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="text-xs text-stone-500">
                          {toVariantLabel(variant) ||
                            "Variant label will build from the fields above."}
                        </div>
                        <Button
                          type="button"
                          variant="ghost"
                  ...
                  <div className="rounded-[22px] border border-dashed border-stone-300 bg-stone-50 px-4 py-5 text-sm text-stone-500">
                    No variants yet. Add colors, sizes, flavors, or models only
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setForm((current) => ({
                          ...
                          images: [
                            ...current.images,
                            emptyImage(current.images.length),
                          ],
                        }))
                      }
                    >
                    ...
                    form.images.map((image, index) => (
                      ...
                      >
                        <div className="grid gap-3 md:grid-cols-[120px_1fr]">
                          <div className="relative h-28 overflow-hidden rounded-[18px] border border-stone-200 bg-white">
                            {image.imageUrl ? (
                              <Image
                                src={image.imageUrl}
                                alt={
                                  image.altText || form.name || "Product image"
                                }
                                fill
              ...
              <div className="rounded-[26px] border border-stone-200 bg-white/70 p-4 sm:p-5">
                ...
                {editingId ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link
                      href={`/print/barcode-labels?productId=${editingId}&size=${labelSize}`}
                      target="_blank"
                    >
                    ...
                    {editingProduct?.variants
                      ...
                      .map((variant) => (
                        <Link
                          key={variant.id}
                          href={`/print/barcode-labels?productId=${editingId}&variantId=${variant.id}&size=${labelSize}`}
                          target="_blank"
                        >
        ...
        <Card>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-500">
            This account can review the catalog
            {canViewPurchaseCosts ? " and purchase-cost data" : ""}, but it does
            not have permission to create, edit, archive, or restore products.
          </p>
        </Card>
      )}
    </div>
