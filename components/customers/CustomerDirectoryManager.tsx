...
function paymentDraftFromCustomer(
...
function getCustomerStats(customer: CustomerRecord) {
  return {
    ...
    totalSpend: customer.sales.reduce(
      (sum, sale) => sum + Number(sale.totalAmount),
      0,
    ),
    loyaltyBalance:
      customer.loyaltyLedger[0]?.balanceAfter ??
      calculateCustomerLoyaltyBalance(customer.loyaltyLedger),
    receivableBalance: customer.creditLedgers.reduce(
...
export default function CustomerDirectoryManager({
  ...
  const agingSummary = useMemo(() => {
    ...
    records.forEach((customer) => {
      customer.creditLedgers.forEach((ledger) => {
        const status = normalizeCustomerCreditStatus(
          ledger.status,
          ledger.dueDate,
  ...
  function changeCustomerType(type: CustomerForm["type"]) {
    setForm((current) => ({
      ...
      type,
      ...(type === "INDIVIDUAL"
        ? { businessName: "", contactPerson: "", taxId: "" }
        : { firstName: "", lastName: "" }),
    }));
  }
  ...
  async function saveCustomer() {
    ...
    const data = await response
      .json()
      .catch(() => ({ error: "Unable to save customer." }));
    setSaving(false);
  ...
  async function postPayment() {
    ...
    const data = await response
      .json()
      .catch(() => ({ error: "Unable to post receivable payment." }));
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          <div className="text-sm text-stone-500">Current</div>
          <div className="mt-2 text-3xl font-black text-stone-900">
            {money(agingSummary.current, currencySymbol)}
          </div>
        </Card>
        <Card>
          <div className="text-sm text-stone-500">1-30 days</div>
          <div className="mt-2 text-3xl font-black text-amber-700">
            {money(agingSummary.days_1_30, currencySymbol)}
          </div>
        </Card>
        <Card>
          <div className="text-sm text-stone-500">31-60 days</div>
          <div className="mt-2 text-3xl font-black text-orange-700">
            {money(agingSummary.days_31_60, currencySymbol)}
          </div>
        </Card>
        <Card>
          <div className="text-sm text-stone-500">61+ days</div>
          <div className="mt-2 text-3xl font-black text-red-700">
            {money(agingSummary.days_61_plus, currencySymbol)}
          </div>
        </Card>
      </div>
        ...
        <Card>
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge tone={customerTypeTone(selectedCustomer.type)}>
          ...
          <div className="grid gap-6 xl:grid-cols-3">
            <div>
              ...
              <div className="space-y-3">
                {selectedCustomer.sales.length ? (
                  selectedCustomer.sales.slice(0, 12).map((sale) => (
                    <div
                      key={sale.id}
            ...
            <div>
              ...
              <div className="space-y-3">
                {selectedCustomer.loyaltyLedger.length ? (
                  selectedCustomer.loyaltyLedger.slice(0, 16).map((entry) => (
                    <div
                      key={entry.id}
                      className="rounded-2xl border border-stone-200 bg-stone-50 p-4"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <div className="font-semibold text-stone-900">
                            {getCustomerLoyaltyTypeLabel(entry.type)}
                          </div>
                          <div className="text-xs text-stone-500">
            ...
            <div>
              ...
              <div className="space-y-3">
                {selectedCustomer.creditLedgers.length ? (
                  selectedCustomer.creditLedgers.map((ledger) => {
                    const status = normalizeCustomerCreditStatus(
                      ledger.status,
                      ledger.dueDate,
                      Number(ledger.balance),
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <div className="font-semibold text-stone-900">
                              {ledger.sale?.saleNumber ?? "Credit sale"}
                            </div>
                            <div className="text-xs text-stone-500">
                              Due {shortDate(ledger.dueDate)}
                            </div>
                          </div>
                          <Badge tone={customerCreditStatusTone(status)}>
                        ...
                        <div className="mt-3 flex justify-between text-sm">
                          ...
                          <span className="font-semibold text-stone-900">
                            {money(ledger.originalAmount, currencySymbol)}
                          </span>
                        </div>
                        <div className="mt-1 flex justify-between text-sm">
                          <span>Balance</span>
                          <span className="font-semibold text-stone-900">
                            {money(ledger.balance, currencySymbol)}
                          </span>
                        </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-6 text-sm text-stone-500">
                    No receivables posted for this customer.
                  </div>
                )}
              </div>

              {selectedCustomer.creditLedgers.some(
                (ledger) => Number(ledger.balance) > 0,
              ) ? (
                <div className="mt-4 rounded-2xl border border-stone-200 bg-white p-4">
                  ...
                  <div className="mt-3 grid gap-3">
                    <select
                      className="rounded-xl border border-stone-300 bg-stone-50 px-4 py-2.5 text-sm"
                      value={paymentDraft.customerCreditLedgerId}
                      onChange={(event) => {
                        const ledger = selectedCustomer.creditLedgers.find(
                          (entry) => entry.id === event.target.value,
                        );
                        setPaymentDraft((current) => ({
                          ...current,
                          customerCreditLedgerId: event.target.value,
                          amount: ledger?.balance ?? current.amount,
                        }));
                      }}
                    >
                      {selectedCustomer.creditLedgers
                        .filter((ledger) => Number(ledger.balance) > 0)
                        .map((ledger) => (
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setPaymentDraft((current) => ({
                          ...current,
                          method: event.target.value as PaymentDraft["method"],
                        }))
                      }
                    >
