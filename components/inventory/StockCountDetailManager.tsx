...
export default function StockCountDetailManager({
  ...
  const [draftLines, setDraftLines] = useState<Record<string, DraftLine>>(() =>
    createDraftLines(initialStockCount.items),
  );
  const [query, setQuery] = useState("");
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const [loadingAction, setLoadingAction] = useState<
    "SAVE" | "SUBMIT" | "APPROVE" | "POST" | "CANCEL" | null
  >(null);

  const canManageApproval =
    currentRole === "MANAGER" || currentRole === "ADMIN";
  const editable =
    ["DRAFT", "IN_PROGRESS"].includes(stockCount.status) &&
  ...
  const draftCountedItemCount = useMemo(
    () =>
      stockCount.items.filter((item) => draftLines[item.id]?.actualQty !== null)
        .length,
        ...
        <Card className="space-y-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge tone={toneForStatus(stockCount.status)}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4">
              ...
              <div className="mt-2 text-3xl font-black text-stone-950">
                {stockCount.revealBlindQuantities
                  ? draftVarianceLineCount
                  : "Hidden"}
              </div>
              <div className="mt-1 text-sm text-stone-500">
                {stockCount.revealBlindQuantities
                  ? "Lines currently off from expectation"
                  : "Blind count is still unreconciled"}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4 text-sm text-stone-600">
            <div>
              ...
              <span className="font-semibold text-stone-900">
                {stockCount.createdByUser.name ??
                  stockCount.createdByUser.email}
              </span>{" "}
              on{" "}
              <span className="font-semibold text-stone-900">
                {dateTime(stockCount.createdAt)}
              </span>
            </div>
            <div className="mt-2">
              {stockCount.approvedByUser ? (
                <>
                  Approved by{" "}
                  <span className="font-semibold text-stone-900">
                    {stockCount.approvedByUser.name ??
                      stockCount.approvedByUser.email}
                  </span>{" "}
                  on{" "}
                  <span className="font-semibold text-stone-900">
                    {stockCount.approvedAt
                      ? dateTime(stockCount.approvedAt)
                      : "N/A"}
                  </span>
                </>
              ) : (
            ...
            <div className="mt-2">
              {stockCount.postedAt ? (
                <>
                  Posted to inventory on{" "}
                  <span className="font-semibold text-stone-900">
                    {dateTime(stockCount.postedAt)}
                  </span>
                </>
                ...
                <>
                  Submitted on{" "}
                  <span className="font-semibold text-stone-900">
                    {dateTime(stockCount.submittedAt)}
                  </span>
                </>
          ...
          {stockCount.isBlind && !stockCount.revealBlindQuantities ? (
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
              Blind count mode is active. Expected quantities and variances stay
              hidden until the count is submitted for reconciliation.
            </div>
          ) : null}
        ...
        <Card className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-stone-950">
                Per-product count entry
              </h2>
              <p className="mt-2 text-sm leading-6 text-stone-500">
                Count each product once, add notes where needed, and reconcile
                only through the approval and posting flow.
              </p>
            </div>
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="bg-stone-50 text-stone-500">
                  <tr>
                    ...
                    <th className="w-36 min-w-36 px-4 py-3.5">Actual</th>
                ...
                <tbody>
                  {filteredItems.map((item) => {
                    ...
                    const varianceQty =
                      actualQty === null ? 0 : actualQty - item.expectedQty;
                        ...
                        <td className="w-36 min-w-36 px-4 py-4">
                          {editable ? (
                            <Input
                              type="number"
                              min="0"
                              className="min-w-[5.5rem] text-center font-semibold tabular-nums"
                              value={
                                actualQty === null ? "" : String(actualQty)
                              }
                              onChange={(event) =>
                            ...
                            <div className="font-semibold text-stone-900">
                              {actualQty ?? "Not counted"}
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-4">
                          <div
                            className={`font-semibold ${stockCount.revealBlindQuantities ? varianceTone(varianceQty) : "text-stone-500"}`}
                          >
                            {stockCount.revealBlindQuantities
                              ? formatVariance(varianceQty)
                              : "Hidden"}
                          </div>
                        </td>
                        <td className="min-w-[220px] px-4 py-4">
                          {editable ? (
                            ...
                            <div className="text-stone-600">
                              {draftLines[item.id]?.note || "No note"}
                            </div>
                          )}
                        </td>
                      </tr>
