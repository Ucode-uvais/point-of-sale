      ...
      <div className="mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-6 py-12 md:grid-cols-2">
        <div className="hidden rounded-3xl bg-linear-to-br from-emerald-600 to-emerald-700 p-10 text-white shadow-xl md:block">
          ...
          <p className="mt-4 text-base leading-7 text-emerald-50">
            Use your business account to access checkout, inventory, reports,
            and branch operations securely.
          </p>
        </div>
        <div className="rounded-3xl border border-stone-200 bg-white p-8 shadow-lg md:p-10">
          ...
          {inactiveAccess ? (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Your shop access is inactive. Contact an administrator if you
              still need access.
            </div>
          ) : null}
          {sessionInvalid ? (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Your previous session does not belong to the currently configured
              database. Sign in again with an account from this database.
            </div>
          ) : null}
            ...
            {showDevAutofill ? (
              <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
                ...
                <div className="mt-1 text-sky-800">
                  Demo credentials are only available in non-production builds.
                </div>
                <button
                  type="button"
                  onClick={() => {
