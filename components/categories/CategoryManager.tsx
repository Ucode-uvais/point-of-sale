  ...
  return (
    <>
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card>
          <h2 id="new-category" className="text-xl font-black text-stone-900">
            Create category
          </h2>
          ...
          <div className="mt-4 space-y-3">
            {categories.length ? (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-2 text-sm text-stone-500">
                          ...
                          {category._count?.children ?? 0} child category(s)
                        </div>
                      </div>
              ...
              <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-500">
                No categories yet. Create one to keep the product catalog
                organized.
              </div>
            )}
          </div>
        ...
        description={
          confirmationCategory ? (
            <>
              ...
              ?
              {confirmationIsDelete &&
              ((confirmationCategory._count?.products ?? 0) > 0 ||
                (confirmationCategory._count?.children ?? 0) > 0) ? (
                <span className="mt-2 block text-red-700">
                  This category currently has{" "}
                  {confirmationCategory._count?.products ?? 0} product(s) and{" "}
                  {confirmationCategory._count?.children ?? 0} child
                  category(s).
                </span>
              ) : null}
            </>
