...
export default function SupplierManager({
  ...
  const [confirmationSupplier, setConfirmationSupplier] =
    ...
    () =>
      suppliers.map((supplier) => {
        ...
        const openInvoiceCount = supplier.invoices.filter(
          (invoice) =>
            invoice.payableEntry && invoice.payableEntry.status !== "PAID",
        ).length;
        const recentPayments = supplier.invoices
          ...
          .sort(
            (left, right) =>
              new Date(right.paidAt).getTime() -
              new Date(left.paidAt).getTime(),
          )
          .slice(0, 3);
  ...
  async function saveSupplier(event: React.FormEvent<HTMLFormElement>) {
    ...
    const data = await response
      .json()
      .catch(() => ({ error: "Failed to save supplier." }));
    if (!response.ok) {
  ...
  function requestSupplierToggle(supplier: Supplier) {
    if (pendingSupplierId) {
      return;
    }

    setError("");
    setConfirmationSupplier(supplier);
  }

  async function confirmSupplierToggle() {
    const supplier = confirmationSupplier;

    if (!supplier || pendingSupplierId) {
      return;
    }
    ...
    try {
      ...
      setError("Unable to update supplier. Please try again.");
      setConfirmationSupplier(null);
    } finally {
  ...
  return (
    <>
      <div className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        <Card>
          <h2 id="new-supplier" className="text-xl font-black text-stone-900">
            Add supplier
          </h2>
          ...
          <div className="mt-4 space-y-4">
            {supplierCards.length ? (
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-2">
                        ...
                        <Badge tone="amber">
                          {supplier._count?.invoices ?? 0} invoice(s)
                        </Badge>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-black text-stone-900">
                        {money(supplier.outstanding, currencySymbol)}
                      </div>
                      <div className="text-xs text-stone-500">
                        {supplier.openInvoiceCount} open invoice(s)
                      </div>
                      <div className="mt-3">
                        <Button
                          type="button"
                          ...
                          onClick={() => requestSupplierToggle(supplier)}
                        >
                          {pendingSupplierId === supplier.id
                            ? "Updating..."
                            : supplier.isActive
                              ? "Archive"
                              : "Restore"}
                  ...
                  <div className="mt-4 grid gap-4 xl:grid-cols-2">
                    <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                      ...
                      <div className="mt-3 space-y-3">
                        {supplier.invoices.filter(
                          (invoice) =>
                            invoice.payableEntry &&
                            invoice.payableEntry.status !== "PAID",
                        ).length ? (
                          supplier.invoices
                            .filter(
                              (invoice) =>
                              ...
                              >
                                <div className="flex items-center justify-between gap-3">
                                  <div>
                                    ...
                                    <div className="text-xs text-stone-500">
                                      {invoice.purchase.purchaseNumber} / Due{" "}
                                      {shortDate(invoice.dueDate)}
                                    </div>
                                  </div>
                                  <Badge
                                    tone={payableStatusTone(
                                      invoice.payableEntry?.status ??
                                ...
                                <div className="mt-2 text-sm text-stone-600">
                                  Balance{" "}
                                  {money(
                                    invoice.payableEntry?.balance ?? "0",
                                    currencySymbol,
                                  )}
                                </div>
                              </div>
                          ...
                          <div className="text-sm text-stone-500">
                            No outstanding supplier payables right now.
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-500">
                No suppliers yet. Add one to start recording purchases against a
                real vendor.
              </div>
            )}
          </div>
      ...
      <ConfirmDialog
        open={Boolean(confirmationSupplier)}
        title={
          confirmationSupplier?.isActive
            ? "Archive supplier?"
            : "Restore supplier?"
        }
        description={
          confirmationSupplier ? (
            <>
              {confirmationSupplier.isActive ? "Archive" : "Restore"}{" "}
              <span className="font-semibold text-stone-900">
                {confirmationSupplier.name}
              </span>
              ?
            </>
          ) : null
        }
        confirmLabel={confirmationSupplier?.isActive ? "Archive" : "Restore"}
        pending={
          Boolean(confirmationSupplier) &&
          pendingSupplierId === confirmationSupplier?.id
        }
        onConfirm={confirmSupplierToggle}
        onCancel={() => setConfirmationSupplier(null)}
      />
    </>
  );
}
