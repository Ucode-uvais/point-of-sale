...
export default function StaffListManager({
  ...
  const activeCount = shopItems.filter((item) => item.isActive).length;
  const inactiveCount = shopItems.length - activeCount;
  const recentlySeenCount = shopItems.filter((item) => {
    if (!item.lastLogin) {
      return false;
    }

    const age = mountedAt - new Date(item.lastLogin).getTime();
    return age >= 0 && age <= 1000 * 60 * 60 * 24 * 7;
  }).length;
  ...
  async function toggleActive(item: SerializedStaffListItem) {
    ...
    const data = await response
      .json()
      .catch(() => ({ error: "Unable to update staff status." }));
    setLoadingId(null);
        ...
        <div className="overflow-hidden rounded-[26px] border border-stone-200">
          ...
          {!filteredItems.length ? (
            <div className="border-t border-stone-200 bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              No staff matched that search. Adjust the filters or create a new
              account.
            </div>
          ) : null}
        </div>
