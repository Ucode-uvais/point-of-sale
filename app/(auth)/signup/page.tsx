...
export default function SignupPage() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSuccess(
      data.message ??
        "Account created. Check your inbox to verify the email address before signing in.",
    );
  ...
  async function resendVerificationEmail() {
    ...
    const data = await response
      .json()
      .catch(() => ({ error: "Unable to resend the verification email." }));
    ...
    setVerificationExpiresAt(data.expiresAt ?? "");
    setResendMessage(
      data.message ?? "A fresh verification email has been sent.",
    );
  }
      ...
      <div className="mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-6 py-12 md:grid-cols-2">
        <div className="hidden rounded-3xl bg-linear-to-br from-emerald-600 to-emerald-700 p-10 text-white shadow-xl md:block">
          ...
          <p className="mt-4 text-base leading-7 text-emerald-50">
            Sign up once, complete the onboarding wizard, add your first
            categories, products, and suppliers, then start selling immediately.
          </p>
        </div>
