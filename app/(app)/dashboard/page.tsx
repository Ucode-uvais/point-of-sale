...
export default async function DashboardPage() {
  ...
  const todaySalesValue =
    ...
    Number(todayRefunds._sum.totalAmount ?? 0);
  const weeklySalesValue =
    Number(weeklySales._sum.totalAmount ?? 0) -
    Number(weeklyRefunds._sum.totalAmount ?? 0);
  const averageTicket =
    todaySaleCount > 0 ? todaySalesValue / todaySaleCount : 0;
  const stockCoverage =
    totalProducts > 0
      ? Math.max(
          0,
  ...
  const criticalLowStockCount = lowStockProducts.filter(
    (product) =>
      product.stockQty <= Math.max(1, Math.floor(product.reorderPoint / 2)),
  ).length;
  const nextSteps = [
    totalProducts === 0
      ? {
          ...
          description:
            "Checkout, barcode search, and inventory alerts become useful as soon as the branch catalog is loaded.",
        }
      : null,
    recentSales.length === 0
      ? {
          ...
          description:
            "Run a live checkout to populate receipts, sales history, cashier metrics, and daily summary alerts.",
        }
      : null,
    showBackOfficeInsights && pendingPurchases === 0
      ? {
          ...
          description:
            "Purchase and receiving records improve stock coverage, valuation, and low-stock guidance.",
        }
      : null,
    showBackOfficeInsights && (!shop.address || !settings?.receiptHeader)
      ? {
          ...
          description:
            "Add receipt identity, contact details, and branch defaults so documents look ready for real customers.",
        }
      : null,
  ].filter(
    (step): step is { href: string; title: string; description: string } =>
      Boolean(step),
  );
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[34px] border border-white/80 bg-[linear-gradient(135deg,rgba(236,253,245,0.92),rgba(255,255,255,0.96),rgba(239,246,255,0.92))] p-6 shadow-[0_32px_100px_-48px_rgba(28,25,23,0.34)] sm:p-8">
        <div className="absolute inset-y-0 right-0 hidden w-1/2 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.18),transparent_58%)] lg:block" />
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.12fr_0.88fr]">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-black tracking-tight text-stone-950 sm:text-[3.3rem]">
              Dashboard
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-stone-600 sm:text-lg">
              Welcome back. {shop.name} is ready for today&apos;s operations,
              with checkout, stock control, and reporting all connected.
            </p>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <div className="rounded-[28px] border border-white/80 bg-white/82 p-4 backdrop-blur">
                ...
                <div className="mt-2 text-2xl font-black text-stone-950">
                  {money(averageTicket, currency)}
                </div>
                <div className="mt-1 text-sm text-stone-500">
                  Based on today&apos;s completed sales.
                </div>
              </div>
              <div className="rounded-[28px] border border-white/80 bg-white/82 p-4 backdrop-blur">
                <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400">
                ...
                <div className="mt-2 text-2xl font-black text-stone-950">
                  {money(weeklySalesValue, currency)}
                </div>
                <div className="mt-1 text-sm text-stone-500">
                  Rolling revenue snapshot for the week.
                </div>
              </div>
              {showBackOfficeInsights ? (
                <div className="rounded-[28px] border border-white/80 bg-white/82 p-4 backdrop-blur">
                  ...
                  <div className="mt-1 text-sm text-stone-500">
                    Open purchase orders still moving through sending or
                    receiving.
                  </div>
                </div>
              ) : null}
          ...
          <Card className="border-white/70 bg-white/88">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="rounded-full border border-emerald-200 bg-emerald-100 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-emerald-700">
                Live
              </div>
            </div>
            ...
            <div className="mt-6 space-y-4">
              <div className="rounded-[26px] bg-[linear-gradient(135deg,#0f172a,#111827,#1f2937)] p-5 text-white shadow-[0_24px_50px_-34px_rgba(15,23,42,0.8)]">
                ...
                <div className="mt-2 text-3xl font-black">
                  {money(todaySalesValue, currency)}
                </div>
                <div className="mt-2 text-sm text-stone-300">
                  {pluralize(todaySaleCount, "sale")} completed so far.
                </div>
              </div>

              <div
                className={`grid gap-3 ${showBackOfficeInsights ? "sm:grid-cols-2" : "sm:grid-cols-1"}`}
              >
                <div className="rounded-3xl border border-stone-200 bg-stone-50/90 p-4">
                  ...
                  <div className="mt-2 text-2xl font-black text-stone-950">
                    {pluralize(lowStockCount, "product")}
                  </div>
                ...
                {showBackOfficeInsights ? (
                  <div className="rounded-3xl border border-stone-200 bg-stone-50/90 p-4">
                    ...
                    <div className="mt-2 text-sm text-stone-500">
                      {pendingPurchases > 0
                        ? "Purchase orders are waiting for more supplier action."
                        : "No open purchase orders are waiting."}
                    </div>
                  </div>
                ) : null}
              </div>
              ...
              <div className="rounded-3xl border border-stone-200 bg-white p-4">
                ...
                <div className="mt-3 h-2.5 rounded-full bg-stone-200">
                  <div
                    className="h-full rounded-full bg-emerald-500 shadow-[0_10px_20px_-12px_rgba(16,185,129,0.9)]"
                    style={{ width: `${stockCoverage}%` }}
                  />
                </div>
                <div className="mt-3 text-sm text-stone-500">
                  Keep an eye on the restock queue so the sales floor stays
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-stone-500">
                These shortcuts only appear while the branch still needs a few
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            {nextSteps.map((step) => (
              <Link
                key={step.href}
                href={step.href}
                className="rounded-[26px] border border-stone-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,245,244,0.92))] p-5 transition hover:-translate-y-0.5 hover:border-emerald-200"
              >
          ...
          <div className="mt-5 space-y-3">
            {recentMovements.length ? (
              recentMovements.map((movement) => (
                <div
                  key={movement.id}
                  className="flex items-center justify-between rounded-3xl border border-stone-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,250,252,0.86))] p-4 transition hover:border-stone-300"
                >
                  <div>
                  ...
                  <div
                    className={`rounded-full px-3 py-1 text-lg font-black ${movement.qtyChange >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                  >
      ...
      <Card>
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black text-stone-950">
              Recent device and session events
            </h2>
          </div>
        </div>
        ...
        <div className="mt-5 space-y-3">
          {recentAuthEvents.length ? (
            recentAuthEvents.map((event) => (
              <div
                key={event.id}
                className="flex items-center justify-between rounded-3xl border border-stone-200 bg-white p-4"
              >
                <div>
                  <div className="font-semibold text-stone-900">
            ...
            <div className="rounded-3xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-500">
              No recent authentication events were recorded for this account
              yet.
            </div>
          )}
        </div>
