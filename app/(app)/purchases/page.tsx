...
export default async function PurchasesPage() {
  ...
  const missingSetup = [
    suppliers.length === 0
      ? { href: "/suppliers", label: "Add at least one supplier" }
      : null,
    products.length === 0
      ? { href: "/products", label: "Add at least one product" }
      : null,
      ...
      {missingSetup.length ? (
        <Card>
          ...
          <h2 className="mt-2 text-xl font-black text-stone-900">
            Purchases are waiting for a few master records
          </h2>
          <p className="mt-2 text-sm text-stone-500">
            This branch can review existing purchase history, but new purchase
            orders should wait until suppliers and products are in place. That
            keeps purchasing operational instead of relying on starter data.
          </p>
      ...
      <PurchaseManager
        ...
        products={products.map((product) => ({
          ...
          baseUnitOfMeasure: product.baseUnitOfMeasure
            ? {
                id: product.baseUnitOfMeasure.id,
                code: product.baseUnitOfMeasure.code,
                name: product.baseUnitOfMeasure.name,
                isBase: product.baseUnitOfMeasure.isBase,
              }
            : null,
          ...
          uomConversions: product.uomConversions.map((conversion) => ({
            ...
            unitOfMeasure: {
              ...
              code: conversion.unitOfMeasure.code,
              name: conversion.unitOfMeasure.name,
              isBase: conversion.unitOfMeasure.isBase,
            },
          })),
        }))}
        units={units}
        purchases={purchases.map(
          (purchase) => serializePurchase(purchase) as unknown as PurchaseView,
        )}
