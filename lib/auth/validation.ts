...
import {
  CREATE_PURCHASE_STATUS_OPTIONS,
  MANUAL_PURCHASE_STATUS_OPTIONS,
} from "@/lib/purchases";
...
export const onboardSchema = z.object({
  ...
  categories: z
    .array(z.object({ name: z.string().trim().min(2).max(60) }))
    .default([]),
  suppliers: z
    .array(
      z.object({
  ...
  uomConversions: z
    .array(
      z.object({
        ...
        ratioToBase: z.coerce
          .number()
          .int()
          .positive("Pack conversion must be greater than zero."),
      }),
    )
    .default([]),
...
export const supplierReturnSchema = z.object({
  ...
  reasonSummary: z
    .string()
    .trim()
    .min(3, "Return summary is required.")
    .max(160),
  notes: z.string().trim().max(500).optional().nullable(),
  ...
  creditMemoStatus: z
    .enum(SUPPLIER_CREDIT_MEMO_STATUS_OPTIONS)
    .default("PENDING"),
  items: z
    .array(
      z.object({
...
export const saleSchema = z
  .object({
    ...
    loyaltyPointsToRedeem: z.coerce
      .number()
      .int()
      .min(0)
      .max(999999)
...
const approvalSchema = z.object({
  approverEmail: z
    .string()
    .trim()
    .email("Enter a valid approver email.")
    .transform((value) => value.toLowerCase()),
...
export const saleVoidSchema = approvalSchema.extend({
  ...
  refundPayments: z
    ...
    .min(1, "Add at least one refund payment line."),
});
...
export const saleRefundSchema = approvalSchema.extend({
  ...
  notes: z.string().trim().max(300).optional().nullable(),
  items: z
    .array(saleAdjustmentReturnItemSchema)
    .min(1, "Select at least one returned item."),
  replacementItems: z
    .array(saleAdjustmentReplacementItemSchema)
    .optional()
    .default([]),
...
export const supplierInvoiceSchema = z.object({
  ...
  totalAmount: z.coerce
    .number()
    .positive("Invoice total must be greater than zero.")
    .max(999999.99),
  notes: z.string().trim().max(500).optional().nullable(),
});
...
export const settingSchema = z.object({
  ...
  offlineStockMaxAgeMinutes: z.coerce
    .number()
    .int()
    .min(5)
    .max(1440)
  ...
  salePrefix: z
    ...
    .min(2)
    .max(10)
    .regex(/^[A-Za-z0-9]+$/, "Use letters or numbers only for the sale prefix"),
  receiptPrefix: z
    .string()
    .trim()
    .min(2)
    ...
    .regex(
      /^[A-Za-z0-9]+$/,
      "Use letters or numbers only for the receipt prefix",
    ),
  purchasePrefix: z
    .string()
    .trim()
    ...
    .regex(
      /^[A-Za-z0-9]+$/,
      "Use letters or numbers only for the purchase prefix",
    ),
});
...
export const cashSessionOpenSchema = z.object({
  openingFloat: z.coerce
    .number()
    .min(0, "Opening float must be zero or greater.")
    .max(999999.99),
  notes: z.string().trim().max(500).optional().nullable(),
});

export const cashSessionCloseSchema = z.object({
  closingActual: z.coerce
    .number()
    .min(0, "Actual counted cash must be zero or greater.")
    .max(999999.99),
  denominationBreakdown: z
    .record(z.string(), z.coerce.number().int().min(0).max(9999))
    .optional()
    .nullable(),
  notes: z.string().trim().max(500).optional().nullable(),
});
...
export const cashMovementSchema = z.object({
  ...
  amount: z.coerce
    .number()
    .positive("Cash movement amount must be greater than zero.")
    .max(999999.99),
  note: z
    .string()
    .trim()
    .min(3, "Add a short note for the cash movement.")
    .max(300),
});
