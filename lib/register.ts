...
export async function getActiveCashSessionsForShop(
  db: RegisterDb,
  shopId: string,
) {
...
export async function buildCashSessionSummary(
    ...
    if (sale.payments.length) {
      for (const payment of sale.payments) {
        paymentTotals.set(
          payment.method,
          roundCurrency(
            (paymentTotals.get(payment.method) ?? 0) + Number(payment.amount),
          ),
        );
      }
    } else {
      paymentTotals.set(
        sale.paymentMethod,
        roundCurrency(
          (paymentTotals.get(sale.paymentMethod) ?? 0) +
  ...
  for (const movement of movements) {
    ...
    movementTotals[movement.type] = roundCurrency(
      movementTotals[movement.type] + amount,
    );
    timeline.push({
